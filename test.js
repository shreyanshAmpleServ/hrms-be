...
/**
 * Fetch hiring stages from database and enrich job posting
 */
const enrichWithHiringStages = async (jobPosting) => {
  ...
  // Fetch actual hiring stage records from database
  ...
  // Maintain the order from the stored string
  const orderedStages = stageIds.map((id) => stageMap.get(id)).filter(Boolean); // Remove any undefined values (deleted stages)
...
const enrichMultipleWithHiringStages = async (jobPostings) => {
  ...
  // Collect all unique hiring stage IDs
  ...
  // Fetch all hiring stages in one query
...
const createJobPosting = async (data) => {
  try {
    ...
    // Enrich with hiring stages data from database
...
const getAllJobPosting = async (search, page, size, startDate, endDate) => {
  try {
    ...
    // Efficiently enrich all job postings with hiring stages in one go
