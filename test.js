...
const urlToBase64 = async (url) => {
  if (!url || url.trim() === "") {
    console.log("No URL provided for image conversion");
    return "";
  }
  ...
  try {
    ...
    console.log("Image converted successfully, size:", buffer.length, "bytes");
    return base64;
  } catch (err) {
 ...
 */
const generatePayslipHTML = async (data, filePath = null) => {
  try {
    // Calculate totals
    ...
    const companyLogo = await urlToBase64(data.company_logo);
    const companySignature = await urlToBase64(data.company_signature);
