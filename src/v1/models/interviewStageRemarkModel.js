...
const checkIfPreviousStagesApproved = async (currentStageId, candidateId) => {
  try {
    ...
    const allPreviousRemarks =
      await prisma.hrms_m_interview_stage_remark.findMany({
        where: {
    ...
    const latestRemarkByStage = new Map();
    allPreviousRemarks.forEach((remark) => {
      const existing = latestRemarkByStage.get(remark.stage_id);
      if (!existing || remark.id > existing.id) {
        latestRemarkByStage.set(remark.stage_id, remark);
      }
    });
    ...
    if (!allPreviousApproved) {
      ...
      const pendingStageNamesSet = new Set(
        pendingRemarks.map(
          (r) =>
      ...
      const pendingStageNames = Array.from(pendingStageNamesSet).join(", ");
...
const createInterviewStageRemark = async (data) => {
  try {
    ...
    const finalResult = await prisma.hrms_m_interview_stage_remark.findUnique({
      where: { id: result.id },
      include: {
        interview_stage_candidate: {
          select: {
...
const checkAndUpdateCandidateStatus = async (candidateId, hiringStageId) => {
  try {
    ...
    const latestRemarks = Array.from(latestRemarkByStage.values());

    console.log(`Found ${latestRemarks.length} latest remarks (unique stages)`);
    latestRemarks.forEach((remark) => {
      console.log(
        `   - Stage ${remark.stage_id}: ${
          remark.interview_stage_remark_hiring_stage?.hiring_stage_hiring_value
            ?.value || "Unknown"
    ...
    const anyRejected = latestRemarks.some(
      (remark) => remark.status === "Rejected" || remark.status === "R"
    );
    ...
    const allApproved =
      latestRemarks.length === stageIds.length &&
      latestRemarks.every(
        (remark) => remark.status === "Approved" || remark.status === "A"
      );
