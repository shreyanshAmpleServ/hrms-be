...
const createGoalCategory = async (data) => {
  try {
    ...
    throw new CustomError(`Error creating goal category: ${error.message}`, 500);
  }
};
...
const findGoalCategoryById = async (id) => {
  try {
    ...
    throw new CustomError(`Error finding goal category by ID: ${error.message}`, 503);
  }
};
...
const updateGoalCategory = async (id, data) => {
  try {
    ...
    throw new CustomError(`Error updating goal category: ${error.message}`, 500);
  }
};
...
const deleteGoalCategory = async (id) => {
  try {
    ...
    throw new CustomError(`Error deleting goal category: ${error.message}`, 500);
  }
};

// Get all goal category
const getAllGoalCategory = async (  page,
  size,
  search,
  startDate,
  endDate) => {
