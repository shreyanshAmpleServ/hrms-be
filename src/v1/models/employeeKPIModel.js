...
const createEmployeeKPI = async (data) => {
  try {
    ...
    console.log("has component_assignment:", !!data.component_assignment);
    if (data.component_assignment) {
      console.log(
        "component_assignment keys:",
        Object.keys(data.component_assignment)
      );
      console.log(
        "component_lines count:",
    ...
    const kpiHeaderId = await prisma.$transaction(
      async (tx) => {
        ...
        console.log(
          " Data Without Status:",
          JSON.stringify(dataWithoutStatus, null, 2)
        );

        console.log("Critical Fields Check:", {
          employee_id: dataWithoutStatus.employee_id,
          employee_id_type: typeof dataWithoutStatus.employee_id,
          reviewer_id: dataWithoutStatus.reviewer_id,
          reviewer_id_type: typeof dataWithoutStatus.reviewer_id,
          log_inst: dataWithoutStatus.log_inst,
          log_inst_type: typeof dataWithoutStatus.log_inst,
        });
            ...
            await tx.hrms_d_employee_kpi_component_assignment.create({
              data: {
                ...
                successor_id: data.component_assignment.successor_id
                  ...
                  : null,
                employment_type_id: data.component_assignment.employment_type_id
                  ? Number(data.component_assignment.employment_type_id)
                  : null,
                contract_expiry_date: data.component_assignment
        ...
        if (!needsWorkflow) {
          console.log(
            `KPI ${kpiHeader.id} created as Active (no workflow configured - auto-approved)`
          );

          await tx.hrms_d_employee_kpi.update({
            where: { id: kpiHeader.id },
          ...
          if (serializedData.employment_type)
            updateEmployeeData.employment_type = serializedData.employment_type;
          if (
            data.revise_component_assignment === "Y" &&
            data.component_assignment
          ) {
            ...
            if (data.component_assignment.position) {
              updateEmployeeData.work_location =
                data.component_assignment.position;
            }
            if (data.component_assignment.employment_type_id) {
          ...
          if (
            ...
            const componentAssignmentRecord =
              await tx.hrms_d_employee_kpi_component_assignment.findFirst({
                where: { employee_kpi_id: kpiHeader.id },
              });

            if (componentAssignmentRecord) {
                ...
                await tx.hrms_d_employee_pay_component_assignment_header.create(
                  {
                    data: {
                      employee_id: Number(data.employee_id),
                      effective_from:
                        componentAssignmentRecord.effective_from || new Date(),
                      effective_to: componentAssignmentRecord.effective_to,
                      department_id: componentAssignmentRecord.department_id,
                      position_id: null,
                      status: "Active",
                      remarks: `Created from Employee KPI #${kpiHeader.id}`,
                      createdby: data.createdby || 1,
                      createdate: new Date(),
                    },
                  }
              ...
              const componentLines =
                await tx.hrms_d_employee_kpi_component_lines.findMany({
                  where: {
                    component_assignment_id: componentAssignmentRecord.id,
                  },
                });
          ...
          if (data.attachments && data.attachments.length > 0) {
            ...
            const attachments =
              await tx.hrms_d_employee_kpi_attachments.findMany({
                where: { employee_kpi_id: kpiHeader.id },
                include: {
            ...
            for (const attachment of attachments) {
              await tx.hrms_d_document_upload.create({
                data: {
                  ...
                  document_owner_id: Number(data.employee_id),
                  createdby: data.createdby || 1,
                  createdate: new Date(),
                },
              });
    ...
    const directCheck =
      await prisma.hrms_d_employee_kpi_component_assignment.findFirst({
        where: { employee_kpi_id: kpiHeaderId },
      });

    console.log("Direct DB Check for component_assignment:", directCheck);
    console.log(
      "Result from findEmployeeKPIById:",
      result.kpi_component_assignment
    );
    ...
    console.log("Error in kpi", error);
...
const approveEmployeeKPI = async (kpiId, approverId) => {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (kpi.kpi_component_assignment?.employment_type_id) {
        updateEmployeeData.employment_type_id =
          kpi.kpi_component_assignment.employment_type_id;
      }
      if (kpi.kpi_component_assignment?.contract_expiry_date) {
        updateEmployeeData.contract_expiry_date =
          kpi.kpi_component_assignment.contract_expiry_date;
      }
...
//     console.log(
//       `KPI Update Workflow Check: needsWorkflow=${needsWorkflow}, updateStatus=${updateStatus}, approverCount=${
//         workflowSteps?.length || 0
//       }`
//     );
...
//         if (!needsWorkflow) {
//           console.log(
//             `KPI ${updatedKPI.id} updated as Active (no workflow configured - auto-approved)`
//           );
...
//             const componentAssignmentRecord =
//               await tx.hrms_d_employee_kpi_component_assignment.findFirst({
//                 where: { employee_kpi_id: updatedKPI.id },
//               });
...
//                       position_id: null,
//                       status: "Active",
//                       remarks: `Created from Employee KPI #${updatedKPI.id}`,
//                       createdby: data.createdby || 1,
//                       createdate: new Date(),
//                     },
//                   }
...
//         } else {
//           console.log(
//             `KPI ${updatedKPI.id} updated as Pending (workflow approval required)`
//           );
//         }
...
// Update the updateEmployeeKPI function
            ...
            await tx.hrms_d_employee_kpi_component_assignment.create({
              data: {
                ...
                successor_id: data.component_assignment.successor_id
                  ? Number(data.component_assignment.successor_id)
                  : null,
                // ADD THESE TWO NEW FIELDS IN UPDATE
                employment_type_id: data.component_assignment.employment_type_id
                  ? Number(data.component_assignment.employment_type_id)
                  : null,
                contract_expiry_date: data.component_assignment
...
const deleteEmployeeKPI = async (id) => {
  try {
    ...
    return await prisma.$transaction(
      async (tx) => {
        ...
        if (componentAssignments.length > 0) {
          const assignmentIds = componentAssignments.map((a) => a.id);
          await tx.hrms_d_employee_kpi_component_lines.deleteMany({
            where: {
              component_assignment_id: { in: assignmentIds },
            },
        ...
        return { id: kpiId, message: "Employee KPI deleted successfully" };
      },
      {
        timeout: 30000,
    ...
    if (
      ...
      throw new CustomError(
        "Transaction failed. Please try again. If the issue persists, the KPI may have been partially deleted.",
        500
      );
    }
