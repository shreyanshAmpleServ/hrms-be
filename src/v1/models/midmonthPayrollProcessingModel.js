...
//           await prisma.hrms_d_monthly_payroll_processing.create({
//             data: record,
//           });
//         } catch (err) {
//           errorResults.push({
...
const createMidMonthPayrollProcessing = async (dataArray) => {
  try {
    ...
    console.log(`Processing ${dataArray.length} mid-month payroll records...`);
    ...
    for (const data of dataArray) {
      try {
        ...
        const serializedData = serializeMidmonthPayrollData(data);
        ...
        if (columnCheck.length === 0) {
          ...
          `);
          console.log(`Added component column [${componentId}]`);
        }
        ...
        if (existingRecord && existingRecord.length > 0) {
          ...
          console.log(
            ` Updated mid-month payroll for employee ${data.employee_id}, component ${componentId}`
          );
        } else {
          const insertQuery = `
          ...
          console.log(
            ` Created mid-month payroll for employee ${data.employee_id}, component ${componentId}`
          );
        }
    ...
    console.log(`\n=== Mid-Month Payroll Processing Complete ===`);
    ...
    console.error("Bulk mid-month payroll processing error:", error);
    throw new CustomError(
      `Error creating mid-month payroll records: ${error.message}`,
      500
    );
