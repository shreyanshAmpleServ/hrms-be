...
const getP09ReportData = async (fromDate, toDate) => {
  try {
    ...
    // Get sample data without hardcoded component columns to avoid errors
    const sampleData = await prisma.$queryRaw`
      SELECT TOP 5 *, 
        CAST(payroll_month AS VARCHAR) + '/' + CAST(payroll_year AS VARCHAR) as payroll_period,
        doc_date,
    ...
    // If the error is about invalid column names, provide fallback data
    if (error.message && error.message.includes("Invalid column name")) {
      console.log(
        "P09 Report - Stored procedure has invalid column references, using fallback data",
      );

      // Get fallback data when stored procedure fails
      try {
        const fallbackData = await prisma.$queryRaw`
          ...
          WHERE doc_date >= ${fromDate} AND doc_date <= ${toDate}
        `;
        console.log("P09 Report - Using fallback data:", fallbackData);
        return fallbackData;
      } catch (fallbackError) {
        console.error(
          "P09 Report - Even fallback query failed:",
          fallbackError,
        );
        throw new CustomError(
          `P09 Report: Both stored procedure and fallback failed. Original error: ${error.message}`,
          500,
        );
      }
    }
            ...
            ${transformedData
              .map(
                ...
                <tr>
                    ...
                    <td class="text-right">${formatTZNumber(
                      item["BASIC PAY"],
                    )}</td>
                    <td class="text-right">${formatTZNumber(
...
<tr class="total-row no-border">
                <td colspan="4">Grand Total :</td>
                <td class="text-right">${formatTZNumber(
                  grandTotal["BASIC PAY"],
                )}</td>
                <td class="text-right">${formatTZNumber(
                  grandTotal["Contribution (20%)"],
                )}</td>
            </tr>
        </tbody>
    </table>
