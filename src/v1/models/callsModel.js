    ...
    try {
        ...
        const calls = await prisma.crms_calls.create({
            data: {
                ...data,
                createdate: new Date(),
                call_start_date: combineDateAndTime(data.call_start_date,data.call_start_time),
                updatedate: new Date(),
                createdby: user?.id || 1,
        ...
        (async () => {
            try {
                if (calls.assigned_to_user?.email && data?.ongoing_callStatus === "Scheduled" && (calls.reminder_type === "Email" || calls.reminder_type === "Both")) {
                await sendEmailNotification(calls.assigned_to_user, calls);
...
const updateCalls = async (id, data) => {
    try {
        ...
        (async () => {
            try {
                if (updatedCalls.assigned_to_user?.email && data?.ongoing_callStatus === "Scheduled" && (updatedCalls.reminder_type === "Email" || updatedCalls.reminder_type === "Both")) {
                await sendEmailNotification(updatedCalls.assigned_to_user, updatedCalls);
...
const getAllCalls = async (data) => {
    ...
    if (data?.search) {
      filters.OR = [
        {
            created_by_user: {
                full_name: { contains: data.search.toLowerCase() },
            },
        },
        {
            crms_m_contact_call_for: {
    ...
    if (data?.startDate && data?.endDate) {
      ...
      if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
        ...
        const finalCalls = calls.map(activity => {
            ...
            const combinedDueDateTime = call_start_date && call_start_time ? combineDateAndTime(call_start_date, call_start_time) : null;
            return {
                ...rest,
                call_start_date: combinedDueDateTime ? combinedDueDateTime : call_start_date, 
