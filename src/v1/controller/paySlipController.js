...
const createPaySlip = async (req, res, next) => {
    ...
    let imageUrl = null;
    if (req.file) {
      imageUrl = await uploadToBackblaze(req.file.buffer, req.file.originalname, req.file.mimetype , "paySlip");
    }
        const data = {
            ...req.body,
...
const getAllPaySlip = async (req, res, next) => {
    try {
        ...
        const data = await paySlipService.getAllPaySlip(search,Number(page), Number(size),startDate && moment(startDate),endDate && moment(endDate));
