...
const updateRatingScale = async (req, res, next) => {
    try {
        ...
        const data = await ratingScaleService.updateRatingScale(req.params.id, reqData);
...
const getAllRatingScale = async (req, res, next) => {
    try {
        ...
        const data = await ratingScaleService.getAllRatingScale(Number(page), Number(size) ,search ,moment(startDate), moment(endDate));
        res.status(200).success(null, data);
    } catch (error) {
        next(error);
    }
