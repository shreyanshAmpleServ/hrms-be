...
const updateAssetsType = async (req, res, next) => {
    try {
        ...
        const data = await assetsTypeService.updateAssetsType(req.params.id, reqData);
...
const getAllAssetsType = async (req, res, next) => {
    try {
        ...
        const data = await assetsTypeService.getAllAssetsType(Number(page), Number(size) ,search ,moment(startDate), moment(endDate));
        res.status(200).success(null, data);
    } catch (error) {
        next(error);
    }
