...
const createActivitiesStatus = async (req, res, next) => {
  try {
    const activitiesStatus = await activitiesService.createActivitiesStatus(
      req.body
    );
    res
      .status(201)
      .success("Activities  created successfully", activitiesStatus);
  } catch (error) {
    next(error);
  }
};
...
const getAllActivitiesStatuses = async (req, res, next) => {
  try {
    const activitiesStatuses = await activitiesService.getAllActivitiesStatuses(
      req.query
    );
...
const getGroupActivities = async (req, res, next) => {
  try {
    const activitiesStatuses = await activitiesService.getGroupActivities(
      req.query
    );
    res.status(200).success(null, activitiesStatuses);
  } catch (error) {
...
const updateActivities = async (req, res, next) => {
  try {
    const activities = await activitiesService.updateActivities(
      req.params.id,
      req.body
    );
