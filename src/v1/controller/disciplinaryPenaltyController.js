...
const createDisciplinaryPenalty = async (req, res, next) => {
    try {
        ...
        const data = await disciplinaryPenaltyService.createDisciplinaryPenalty(reqData);
...
const findDisciplinaryPenaltyById = async (req, res, next) => {
    try {
        const data = await disciplinaryPenaltyService.findDisciplinaryPenaltyById(req.params.id);
...
const updateDisciplinaryPenalty = async (req, res, next) => {
    try {
        ...
        const data = await disciplinaryPenaltyService.updateDisciplinaryPenalty(req.params.id, reqData);
...
const getAllDisciplinaryPenalty = async (req, res, next) => {
    try {
        ...
        const data = await disciplinaryPenaltyService.getAllDisciplinaryPenalty(Number(page), Number(size) ,search ,moment(startDate), moment(endDate));
        res.status(200).success(null, data);
    } catch (error) {
        next(error);
    }
