...
const findDesignationById = async (req, res, next) => {
    try {
        const designation = await designationService.findDesignationById(req.params.id);
...
const updateDesignation = async (req, res, next) => {
    try {
        let designationData = { ...req.body };
        const designation = await designationService.updateDesignation(req.params.id, designationData);
...
const getAllDesignation = async (req, res, next) => {
    try {
        ...
        const designations = await designationService.getAllDesignation(Number(page), Number(size) ,search ,moment(startDate), moment(endDate));
