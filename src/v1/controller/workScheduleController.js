...
const updateWorkSchedule = async (req, res, next) => {
    try {
        ...
        const data = await workScheduleService.updateWorkSchedule(req.params.id, reqData);
...
const getAllWorkSchedule = async (req, res, next) => {
    try {
        ...
        const data = await workScheduleService.getAllWorkSchedule(Number(page), Number(size) ,search ,moment(startDate), moment(endDate));
        res.status(200).success(null, data);
    } catch (error) {
        next(error);
    }
