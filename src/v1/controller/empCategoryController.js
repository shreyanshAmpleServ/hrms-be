...
const createEmpCategory = async (req, res, next) => {
  try {
    ...
    const empCategory = await empCategoryService.createEmpCategory(
      empCategoryData
    );
    res
      .status(201)
      .success("Employee category created successfully", empCategory);
  } catch (error) {
    next(error);
  }
};
...
const findEmpCategoryById = async (req, res, next) => {
  try {
    const empCategory = await empCategoryService.findEmpCategoryById(
      req.params.id
    );
...
const updateEmpCategory = async (req, res, next) => {
  try {
    let empCategoryData = { ...req.body };
    const designation = await empCategoryService.updateEmpCategory(
      req.params.id,
      empCategoryData
    );
    res
      .status(200)
      .success("Employee category updated successfully", designation);
  } catch (error) {
    next(error);
  }
};
...
const getAllEmpCategory = async (req, res, next) => {
  try {
    ...
    const empCategories = await empCategoryService.getAllEmpCategory(
      Number(page),
      Number(size),
      search,
      moment(startDate),
