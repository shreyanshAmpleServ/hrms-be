...
const createSalaryStructure = async (req, res, next) => {
    try {
        ...
        const salaryStructure = await salaryStructureService.createSalaryStructure(salaryData);
        res.status(201).success('Salary structure created successfully', salaryStructure);
    } catch (error) {
        next(error);
    }
};
...
const findSalaryStructureById = async (req, res, next) => {
    try {
        const salaryStructure = await salaryStructureService.findSalaryStructureById(req.params.id);
        if (!salaryStructure) throw new CustomError('Salary structure not found', 404);
...
const updateSalaryStructure = async (req, res, next) => {
    try {
        ...
        const salaryStructure = await salaryStructureService.updateSalaryStructure(req.params.id, salaryData);
        res.status(200).success('Salary structure updated successfully', salaryStructure);
    } catch (error) {
        next(error);
    }
};
...
const getAllSalaryStructure = async (req, res, next) => {
    try {
        ...
        const salaryStructures = await salaryStructureService.getAllSalaryStructure(Number(page), Number(size) ,search ,moment(startDate), moment(endDate));
