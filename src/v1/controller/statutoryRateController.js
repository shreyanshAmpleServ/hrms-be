...
const createStatutoryRate = async (req, res, next) => {
    try {
        ...
        const statutoryRate = await statutoryRateService.createStatutoryRate(StatutoryData);
        res.status(201).success('Statutory rate created successfully', statutoryRate);
    } catch (error) {
        next(error);
    }
};
...
const findStatutoryRateById = async (req, res, next) => {
    try {
        const statutoryRate = await statutoryRateService.findStatutoryRateById(req.params.id);
...
const updateStatutoryRate = async (req, res, next) => {
    try {
        ...
        const statutoryRate = await statutoryRateService.updateStatutoryRate(req.params.id, StatutoryData);
        res.status(200).success('Statutory rate updated successfully', statutoryRate);
    } catch (error) {
        next(error);
    }
};
...
const getAllStatutoryRate = async (req, res, next) => {
    try {
        ...
        const statutoryRate = await statutoryRateService.getAllStatutoryRate(Number(page), Number(size) ,search ,moment(startDate), moment(endDate));
        res.status(200).success(null, statutoryRate);
    } catch (error) {
        next(error);
    }
