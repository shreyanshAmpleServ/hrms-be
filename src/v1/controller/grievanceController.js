...
const createGrievanceSubmission = async (req, res, next) => {
    try {
        ...
        res.status(201).success('Grievance submission created successfully', reqData);
    } catch (error) {
        next(error);
    }
};
...
const findGrievanceSubmissionById = async (req, res, next) => {
    try {
        const reqData = await grievanceService.findGrievanceSubmissionById(req.params.id);
...
const updateGrievanceSubmission = async (req, res, next) => {
    try {
        const data = {
            ...
            log_inst: req.user.log_inst,  }
        const reqData = await grievanceService.updateGrievanceSubmission(req.params.id, data);
        res.status(200).success('Grievance submission updated successfully', reqData);
    } catch (error) {
        next(error);
    }
};
...
const getAllGrievanceSubmission = async (req, res, next) => {
    try {
        ...
        const data = await grievanceService.getAllGrievanceSubmission(search,Number(page), Number(size),startDate && moment(startDate),endDate && moment(endDate));
